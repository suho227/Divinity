...
export async function generateMetadata({ params }: PageProps) {
    ...
    const t = await getTranslations({ locale, namespace: 'AdmissionForms' });
...
export default async function AdmissionFormsPage({ params }: PageProps) {
    ...
    return (
        <main className="min-h-screen bg-[#f3f4f9] py-16 px-4 md:py-24">
            <section className="mx-auto max-w-5xl rounded-3xl bg-white px-6 py-10 shadow-sm md:px-10 lg:px-12">
                <div className="min-w-0 pt-2">
                    <div className="mb-7 flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
                        <div>
                            ...
                            <p className="mt-6 text-sm text-slate-500">
                                <span className="font-bold text-[#2d6fdf]">{t('newLabel')} {notices.filter((notice) => notice.isImportant).length}</span>
                        ...
                        {notices.length > 0 ? (
                            <ol className="divide-y divide-slate-200">
                                {notices.map((notice, index) => {
                                    ...
                                    return (
                                        <li key={notice._id}>
                                            <Link href={`/admission/forms/${notice.slug}`} className="grid gap-3 px-2 py-5 text-sm transition-colors hover:bg-slate-50 sm:grid-cols-[56px_1fr_120px] sm:items-center md:px-7">
                                                <span className="text-center text-slate-500">{number}</span>
                                                <span className="break-keep font-medium text-slate-900">
                                                    {title}
                                                    {notice.isImportant && <span className="ml-2 rounded bg-[#2d6fdf] px-2 py-0.5 text-[10px] font-bold text-white">NEW</span>}
                                                </span>
                                                <time className="text-slate-500 sm:text-right" dateTime={notice.publishedAt}>
                                                    {notice.publishedAt ? new Date(notice.publishedAt).toLocaleDateString(locale) : ''}
                                                </time>
                                            </Link>
                                        </li>
                                    );
                    ...
                    <div className="mt-10 flex justify-center">
                        <span className="inline-flex size-11 items-center justify-center border border-[#2d6fdf] text-sm font-medium text-[#2d6fdf]">1</span>
                    </div>
                </div>
            </section>
        </main>
